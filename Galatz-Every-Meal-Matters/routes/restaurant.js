...
router.post('/', auth, async (req, res) => {
    ...
    try {
        ...
        var insertQuery = "INSERT INTO restaurant (name,description,email,address,contact,starRating,type,image) VALUES ?";
...
//search - done
router.post('/hello/search/',async(req,res)=>{
    try {
    ...
    let x= await query('SELECT * FROM restaurant where name like ?','%'+req.body.search+'%');
    console.log(x, "heth"); 
    res.send(x)
...
router.post('/filter',async(req,res)=>{
    try {
        ...
        const query1 = util.promisify(connection.query).bind(connection);
        // const restaurant = await Restaurant.findById(req.params.id);
        let {description, avgCost}=req.body;
        const sql1 = await query1(
            `SELECT DISTINCT(r.restaurantId), r.name, r.address, r.contact, r.timings, r.date,r.starRating, r.type, r.description, r.email, r.isAvailable, r.image 
            FROM restaurant r JOIN menuItems m ON r.restaurantId = m.restaurant 
            WHERE m.description like '%${description}%' AND 
            (SELECT AVG(Cost) FROM menuItems m2 WHERE m2.restaurant=r.restaurantId )>${avgCost}`
            );
        console.log(sql1)
        res.send(sql1)
    } catch (error) {
...
router.get('/filter/:date',async(req,res)=>{
    try {
        ...
        const sql1 = await query1(
            `SELECT SUM(o.orderCost) AS SpentCost, u.name as userName,r.name, o.orderDate
            FROM user u JOIN orderHistory o USING(userId) JOIN restaurant r using(restaurantId)
            WHERE o.orderDate LIKE '${req.params.date}%'
            GROUP BY o.orderDate, u.name, r.name
